...
function Heart() {
  ...
  const playAudio = () => {
    ...
    if (drawBox) {
      drawBox.removeAttribute("onClick"); // Prevent multiple plays
    }
  };

  // Small heart animation using GSAP
  const smallHeartAnim = () => {
    const tl = gsap.timeline();
    tl.set(".smallHeart", { opacity: 1 }).fromTo(
      ".smallHeart",
      {
        scale: 0,
        rotate: "-=25",
  ...
  // Function to create hearts dynamically
  const createHeart = () => {
    const heart = document.createElement("div");
    heart.classList.add("heart");
    heart.style.left = Math.random() * 100 + "vw"; // Random horizontal position
    heart.style.animationDuration = Math.random() * 1.5 + 2 + "s"; // Randomize fall speed

    document.body.appendChild(heart);

    // Remove heart after animation ends (duration is based on animation time)
    setTimeout(() => {
      heart.remove();
    }, 60000); // Match with animation time (2.5s in fall animation)
  };
